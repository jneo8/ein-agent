...
async def lifespan(app: FastAPI):
    ...
    logger.configure(extra={"service": "ein-agent"})
    ...
    temporal_queue = os.getenv("TEMPORAL_QUEUE", "ein-agent-queue")
...
app = FastAPI(title="Ein Agent API", version="0.1.0", lifespan=lifespan)


@app.get("/")
async def root() -> Dict[str, str]:
    """Root endpoint."""
    return {"message": "Ein Agent API is running"}
